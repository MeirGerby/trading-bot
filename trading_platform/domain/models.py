...
@dataclass(frozen=True)
class IdeaAction:
    """A concrete change applied (or to be applied) by the Creative Advisor Agent."""
    action_type: str  # "set_strategy_param" | "set_risk_param" | "add_watchlist" | "remove_watchlist"
    key: str          # param name or ticker symbol
    new_value: str    # string for JSON-safety; parse to float when applying numeric params
    old_value: str    # previous value for audit trail


@dataclass(frozen=True)
class Idea:
    """A structured improvement suggestion generated by IdeaEngine.

    Applied immediately upon generation; `applied` is True once the action
    has been written to memory / symbol repository.
    """
    id: str
    category: str     # "strategy" | "risk" | "watchlist" | "market_context"
    title: str
    rationale: str
    source: str       # "performance_analysis" | "market_data" | "lessons" | "critiques" | "watchlist_health"
    priority: str     # "high" | "medium" | "low"
    actions: tuple[IdeaAction, ...]
    generated_at: datetime
    applied: bool = False
