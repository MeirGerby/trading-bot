...
| 3 | Market data layer — yfinance adapter behind `MarketDataPort`, caching, rate-limit handling | ⏳ Next |
| 4 | Memory layer — `MemoryStore` port wrapping JSON persistence (weights, feedback, alerts, knowledge base) | Planned |
| 5 | Strategy engine — migrate breakout/momentum/options into pluggable `Strategy` implementations; add registry | Planned |
